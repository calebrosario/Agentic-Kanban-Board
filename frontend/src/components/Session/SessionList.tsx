      ...
      <div className="p-3 space-y-2 flex-1 overflow-y-auto">
        {filteredSessions.length === 0 ? (
          <div className="text-center py-8 text-gray-500">
            <p className="text-sm">{t('session.list.empty.title')}</p>
          </div>
        ) : (
...
export const SessionList: React.FC<SessionListProps> = ({ onCreateSession }) => {
  ...
  const handleInterrupt = async (sessionId: string) => {
    try {
      ...
      toast.success(t('session.list.status.interrupted'));
    } catch (error) {
  ...
  const handleDelete = async (sessionId: string) => {
    ...
    try {
      ...
      toast.success(t('session.list.status.deleted'));
    } catch (error) {
      toast.error(t('session.list.error.cantDelete'));
    }
  };
    ...
    <div className={`h-full flex flex-col min-w-0`}>
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={onCreateSession} className="btn-primary flex items-center gap-2 text-sm">
              ...
              <span className="hidden sm:inline">{t('session.list.createButton')}</span>
            </button>
          </div>
        </div>
      ...
      <div className="flex-1 overflow-auto">
        <div className={`h-full ${deviceType === "mobile" ? "p-4" : "p-6"}`}>
          {/* Kanban 看板 */}
          {sessions.length === 0 ? (
            <EmptyState title={t('session.list.empty.title')} description={t('session.list.empty.description')} actionText={t('session.list.empty.createButton')} onAction={onCreateSession} />
          ) : (
            <>
              {/* 行動版標籤頁 */}
              {deviceType === "mobile" && (
                <div className="glass-card rounded-xl shadow-soft p-1 mb-4 overflow-x-auto">
                  <div className="flex">
                    <button onClick={() => setActiveTab("processing")} className={`px-4 py-2 text-sm font-medium whitespace-nowrap rounded-lg transition-all ${activeTab === "processing" ? "bg-warning-100 text-warning-700 shadow-soft-sm" : "text-gray-500 hover:text-gray-700 hover:bg-gray-50"}`}>
                      {t('session.list.tabs.processing')} ({sessionsByStatus[SessionStatus.PROCESSING]?.length || 0})
                    </button>
                    <button onClick={() => setActiveTab("idle")} className={`px-4 py-2 text-sm font-medium whitespace-nowrap rounded-lg transition-all ${activeTab === "idle" ? "bg-success-100 text-success-700 shadow-soft-sm" : "text-gray-500 hover:text-gray-700 hover:bg-gray-50"}`}>
                      {t('session.list.tabs.idle')} ({sessionsByStatus[SessionStatus.IDLE]?.length || 0})
                    </button>
                    <button onClick={() => setActiveTab("completed")} className={`px-4 py-2 text-sm font-medium whitespace-nowrap rounded-lg transition-all ${activeTab === "completed" ? "bg-primary-100 text-primary-700 shadow-soft-sm" : "text-gray-500 hover:text-gray-700 hover:bg-gray-50"}`}>
                      {t('session.list.tabs.completed')} ({[...(sessionsByStatus[SessionStatus.COMPLETED] || []), ...(sessionsByStatus[SessionStatus.ERROR] || []), ...(sessionsByStatus[SessionStatus.INTERRUPTED] || [])].length})
                    </button>
                  </div>
                </div>
              )}
              ...
              <div className={deviceType === "mobile" ? "space-y-4 w-full" : "grid grid-cols-3 gap-4 md:gap-6 min-w-[900px]"}>
                ...
                {deviceType === "mobile" ? (
                  <>
                    {activeTab === "processing" && <KanbanColumn title={t('session.list.tabs.processing')} color="yellow" sessions={sessionsByStatus[SessionStatus.PROCESSING] || []} searchTerm={searchTerm} sortType={sortType} onComplete={handleComplete} onInterrupt={handleInterrupt} onResume={handleResume} onDelete={handleDelete} disableDrag={true} />}

                    {activeTab === "idle" && <KanbanColumn title={t('session.list.tabs.idle')} color="green" sessions={sessionsByStatus[SessionStatus.IDLE] || []} searchTerm={searchTerm} sortType={sortType} onComplete={handleComplete} onInterrupt={handleInterrupt} onResume={handleResume} onDelete={handleDelete} onReorder={(reorderedSessions) => reorderSessionsByStatus(SessionStatus.IDLE, reorderedSessions)} />}

                    {activeTab === "completed" && (
                      <KanbanColumn
                        title={t('session.list.tabs.completed')}
                        color="blue"
                  ...
                  <>
                    {/* 桌面版和平板版：顯示所有欄位 */}
                    <KanbanColumn title={t('session.list.tabs.processing')} color="yellow" sessions={sessionsByStatus[SessionStatus.PROCESSING] || []} searchTerm={searchTerm} sortType={sortType} onComplete={handleComplete} onInterrupt={handleInterrupt} onResume={handleResume} onDelete={handleDelete} disableDrag={true} />

                    <KanbanColumn title={t('session.list.tabs.idle')} color="green" sessions={sessionsByStatus[SessionStatus.IDLE] || []} searchTerm={searchTerm} sortType={sortType} onComplete={handleComplete} onInterrupt={handleInterrupt} onResume={handleResume} onDelete={handleDelete} onReorder={(reorderedSessions) => reorderSessionsByStatus(SessionStatus.IDLE, reorderedSessions)} />
